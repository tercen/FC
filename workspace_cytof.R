...
cluster_PhenoGraph <- cytof_cluster(xdata = data, method = "Rphenograph")

## run ClusterX
cluster_ClusterX <- cytof_cluster(ydata = data_tsne,  method="ClusterX")

cluster_DensVM <- cytof_cluster(xdata = data, ydata = data_tsne, method = "DensVM")
## run FlowSOM with cluster number 15
cluster_FlowSOM <- cytof_cluster(xdata = data, method = "FlowSOM", FlowSOM_k = 12)

## combine data
data_all <- cbind(data, data_tsne, 
                  PhenoGraph = cluster_PhenoGraph, ClusterX=cluster_ClusterX, 
                  FlowSOM=cluster_FlowSOM)
